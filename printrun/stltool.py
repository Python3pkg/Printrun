...
def ray_triangle_intersection(ray_near, ray_dir, xxx_todo_changeme):
    """
    Möller–Trumbore intersection algorithm in pure python
    Based on http://en.wikipedia.org/wiki/M%C3%B6ller%E2%80%93Trumbore_intersection_algorithm
    """
    (v1, v2, v3) = xxx_todo_changeme
    eps = 0.000001
    edge1 = v2 - v1
    edge2 = v3 - v1
...
class stl(object):
    ...
    def parseline(self, l):
        ...
        if l.startswith("solid"):
            ...
            self.infacet = 1
            self.facetloc = 0
            normal = numpy.array(list(map(float, l.split()[2:])))
            ...
            l = l.replace(", ", ".")
            self.facet[1][self.facetloc][:] = numpy.array(list(map(float, l.split()[1:])))
            self.facetloc += 1
        return 1
